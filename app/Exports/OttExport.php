...
class OttExport implements FromArray
{
    public function __construct($sortOrder, $sortField, $request)
    {
    ...
    public function array(): array
    {
        //echo $this->sortOrder;
        //die();
        $query = OttForm::orderBy($this->sortField, $this->sortOrder);
        $query->where('step', '8');
        // if (!$this->request->has('showall') && $this->request->showall != 1) {
        //     //die();

        //     $query->where('step', '8');
        // }
        // if ($this->request->has('title') &&  $this->request->title != '') {
        //     $query->where('title', 'like', '%' .  $this->request->title . '%');
        // }

        // if ($this->request->has('title_in_english') &&  $this->request->title_in_english != '') {

        //     $query->where('title_in_english', 'like', '%' .  $this->request->title_in_english . '%');
        // }

        // if ($this->request->has('client_id') &&  $this->request->client_id != '') {

        //     $query->where('client_id',  $this->request->client_id);
        // }

        // if ($this->request->has('genre_id') && !empty($this->request->genre_id)) {
        //     $query->whereIn('genre_id',  $this->request->genre_id);
        // }

        // if ($this->request->has('language_id') &&  $this->request->language_id != '') {
        //     $query->where('language_id',  $this->request->language_id);
        // }
        // if ($this->request->has('created_at_start') && $this->request->created_at_start != '') {
        //     $query->whereDate('created_at', '>=', $this->request->created_at_start);
        // }

        // if ($this->request->has('created_at_end') && $this->request->created_at_end != '') {
